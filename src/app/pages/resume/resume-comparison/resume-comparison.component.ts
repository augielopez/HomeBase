...
import { ResumeGapAnalysisService } from '../../resume-services/resume-gap-analysis.service';
...
export class ResumeComparisonComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (navigation.masterResume && navigation.tailoredResume && navigation.jobDescription) {
      ...
      setTimeout(() => this.router.navigate(['/dashboard/pages/resume/tailoring']), 2000);
    }
  }
  ...
  goBack() {
    this.router.navigate(['/dashboard/pages/resume/tailoring']);
  }
  ...
  onViewModeChange() {
    if (this.viewMode === 'edit') {
      ...
      this.router.navigate(['/dashboard/pages/resume/experience']);
    }
  }
