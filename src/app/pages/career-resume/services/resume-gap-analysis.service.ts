...
export class ResumeGapAnalysisService {
  ...
  private compareSkills(
    ...
    const masterSkillNames = new Set(masterResume.skills.map((skill: ResumeSkill) => skill.name.toLowerCase()));
    const tailoredSkillNames = new Set(tailoredResume.skills.map((skill: ResumeSkill) => skill.name.toLowerCase()));
    ...
    const allRequiredSkills = new Set([
      ...jobSkills,
      ...tailoredResume.skills.map((skill: ResumeSkill) => skill.name)
    ]);
    ...
    const requiredSkills: SkillMatch[] = Array.from(allRequiredSkills).map(skill => {
      ...
      const alternatives = this.findAlternativeSkills(skill, Array.from(masterSkillNames) as string[]);
    ...
    const matchedSkills = requiredSkills.filter((skill: SkillMatch) => skill.status === 'matched');
    const missingSkills = requiredSkills.filter((skill: SkillMatch) => skill.status === 'missing');
  ...
  private analyzeExperience(
    ...
    tailoredResume.experience.forEach((exp: ResumeExperience) => {
      exp.responsibilities?.forEach((resp: ResumeResponsibility) => {
        totalBullets++;
  ...
  private flattenResumeToText(resume: TailoredResume): string {
    ...
    parts.push(...resume.skills.map((skill: ResumeSkill) => skill.name));
    resume.experience.forEach((exp: ResumeExperience) => {
      parts.push(exp.role);
      parts.push(exp.company);
      exp.responsibilities?.forEach((responsibility: ResumeResponsibility) => parts.push(responsibility.description));
    });
    resume.projects.forEach((proj: ResumeProject) => {
  ...
  private generateRecommendations(
    ...
    skillsAnalysis.missingSkills.forEach((skill: SkillMatch) => {
    ...
    skillsAnalysis.requiredSkills.filter((skill: SkillMatch) => skill.status === 'partial').forEach((skill: SkillMatch) => {
      recommendations.push({
        type: 'skill',
        priority: 'high',
    ...
    if (experienceAnalysis.weakBullets.length > 0) {
      experienceAnalysis.weakBullets.slice(0, 5).forEach((bullet: BulletAnalysis) => {
        recommendations.push({
          type: 'experience',
          priority: 'high',
    ...
    const lowKeywords = atsScore.keywordDensity.filter((keyword: KeywordDensity) => keyword.status === 'low');
    ...
    if (atsScore.formattingIssues.length > 0) {
      atsScore.formattingIssues.forEach((issue: string) => {
        recommendations.push({
          type: 'format',
          priority: 'low',
    ...
    recommendations.sort(
      (a: Recommendation, b: Recommendation) => priorityOrder[a.priority] - priorityOrder[b.priority]
    );
  ...
  private extractSkillsFromJobDescription(text: string): string[] {
    ...
    Object.values(skillPatterns).flat().forEach((pattern: string) => {
